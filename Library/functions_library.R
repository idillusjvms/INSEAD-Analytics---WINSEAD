...
hist_den = function(redwine, x_string,graph_title) {
  ...
  # ggplot(redwine, aes(cx=get(x_string))) +
  #   geom_bar()
  #   geom_density(col=2) +     labs(title=graph_title) +
  #   labs(x=x_string, y="Count")
  
  # barplot(table(redwine[,x_string]), main = graph_title, xlab = x_string, ylab = "Count")
